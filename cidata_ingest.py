...
def decrypt_secrets_archive(enc_file):
    ...
    subprocess.run(["chown", "-R", "1000:1000", "/target/home/shane/"], check=True)
    
    # Enforce strict 600 permissions for ingested secrets
    subprocess.run(["find", "/target/home/shane/", "-name", "rclone.conf", "-exec", "chmod", "600", "{}", "+"], check=True)
    subprocess.run(["find", "/target/home/shane/", "-name", "claude.json", "-exec", "chmod", "600", "{}", "+"], check=True)
